...
# ============================================================
# Ensure MRHead sample data is the real ~6.6 MB volume from SlicerTestingData,
# not the 524 KB synthetic stub generated by setup_slicer.sh's fallback when
# the upstream URL was wrong (now fixed). Threshold of 1 MB rejects the stub
# and triggers a real download. Download to a temp file first and only mv on
# success so a failed retry can't clobber an already-valid file.
if [ ! -f "$MRHEAD_FILE" ] || [ $(stat -c%s "$MRHEAD_FILE" 2>/dev/null || echo 0) -lt 1000000 ]; then
    echo "Downloading MRHead sample data..."
    TMP_DL=$(mktemp /tmp/mrhead.XXXXXX.nrrd)
    if curl -L -o "$TMP_DL" --connect-timeout 30 --max-time 120 \
        ...
        && [ $(stat -c%s "$TMP_DL" 2>/dev/null || echo 0) -gt 1000000 ]; then
        mv "$TMP_DL" "$MRHEAD_FILE"
    elif wget -O "$TMP_DL" --timeout=120 \
